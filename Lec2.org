...
#+BEGIN_SRC ipython :session :file Lec2Bode1.svg :exports results
num = [1,1];
den = [1,2,3];
sys = tf(num, den);
bode(sys, dB=True);
...
#+BEGIN_SRC ipython :session :file Lec2Bode2.svg :exports results
num = [1,1];
den = [1,2,3];
sys = tf(num, den);
...
\begin{align}
G(s) = \frac{10(s+1)}{s(s^2+0.4s+1)} &= \frac{2.5\times (s+1)}{s \times \left[(0.5s)^2 + 0.2(0.5s)+1\right]}\\
...
#+BEGIN_SRC ipython :session :file Lec2BodeConstant.svg :exports results
num = [10];
den = [1];
sys = tf(num, den);
mag, phase, omega = control.bode_plot(sys, dB=True, Plot=False);
...
#+BEGIN_SRC ipython :session :file Lec2BodeIntegrators.svg :exports results
num = [1];
den = [1,0];
sys = tf(num, den);
mag, phase, omega = control.bode_plot(sys, dB=True, Plot=False);
...
#+BEGIN_SRC ipython :session :file Lec2BodeIntegrators2.svg :exports results
num1 = [1];
den1 = [1,0];
sys1 = tf(num1, den1);
...
#+RESULTS:
[[file:Lec2BodeIntegrators2.svg]]

** Bode Plot of $1/(\tau s+ 1)$

#+BEGIN_SRC ipython :session :file Lec2BodeFirstOrder.svg :exports results
num = [1];
den = [0.1,1];
sys = tf(num, den);
...
#+BEGIN_SRC ipython :session :file Lec2BodeFirstOrderAsym.svg :exports results
num = [1];
den = [0.1,1];
sys = tf(num, den);
mag, phase, omega = control.bode_plot(sys, dB=True, Plot=False, omega=np.logspace(-1,3,100));
...
#+RESULTS:
[[file:Lec2BodeFirstOrderAsym.svg]]


** Sketch Bode Plot of $\tau s+ 1$ Using Asymptotes

We can flip the plots of $1/(\tau s+1)$ to get the plots of $\tau s+1$
#+BEGIN_SRC ipython :session :file Lec2BodeFirstOrderAsymInv.svg :exports results
num = [1,1];
den = [1];
sys = tf(num, den);
...
#+RESULTS:
[[file:Lec2BodeFirstOrderAsymInv.svg]]
** Bode Plot of $\frac{1}{(s/\omega_n)^2+2\zeta(s/\omega_n)+1}$

Recall that
- $\omega_n$ is called the natural frequency.
- $\zeta$ is the damping ratio.
#+BEGIN_SRC ipython :session :file Lec2BodeSecondOrder.svg :exports results
wn = 10;
...
#+BEGIN_SRC ipython :session :file Lec2BodeSecondOrder2.svg :exports results
wn = 1;
zeta = 0.25;
